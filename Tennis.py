...
class EnvUtils:
    def __init__(self):
        ...
        self.states_normalized = np.empty(
            shape=(1, admin.number_of_agents, admin.state_size))
...
class Administration:
    ...
    def train_ddpg(self):
        ...
        for i in range(self.episodes_train):
            ...
            if (i + 1) % 1 == 0:
                ...
                if i > 99:
                    print('\rscores: <mean over last 100 Episodes> | <last Episode>: min: {:.5f} | {:.5f}\tmean: {:.5f} | {:.5f}\t'
                    ...
                    print('\rscores last episode: min_Score {:.5f} \tAverage_Score: {:.5f} \tMax_Score {:.5f} \tEpisode {}/{}\t'
    ...
    def test(self):
        for i in range(5):  # play game for 5 episodes
            ...
            scores = np.zeros(self.number_of_agents)  # initialize the score (for each agent)
            while True:
                actions = np.random.randn(self.number_of_agents, self.action_size)  # select an action (for each agent)
    ...
    def init_agent(self):
        ...
        env_info = env.reset(train_mode=self.env_train_mode)[brain_name]

        # number of agents
        self.number_of_agents = len(env_info.agents)
